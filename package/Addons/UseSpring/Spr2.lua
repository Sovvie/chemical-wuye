...
--    function setter)
--
--    Creates a new spring-driven value that can be animated.
--    The setter function is called every frame with the updated value.
--
--
-- spr.stop(
--    Instance obj[,
...
-- Frame loop
local springStates_other: {[any]: {[string]: any}} = {} -- {[key] = {[property] = spring}}
local springStates_render: {[any]: {[string]: any}} = {} -- {[key] = {[property] = spring}}
local valueSprings: {[any]: any} = {} -- {[setter] = spring}

local completedCallbacks: {[any]: {()->()}} = {}

local function processSprings(springStates: typeof(springStates_other), dt: number)
	for key, state in springStates do
		for propName, spring in state do
			if spring:canSleep() then
				state[propName] = nil
				if typeof(key) == "Instance" then
					setProperty(key, propName, spring.rawGoal)
				end
			else
				local value = spring:step(dt)
				if typeof(key) == "Instance" then
					setProperty(key, propName, value)
				end
			end
		end

		if not next(state) then
			springStates[key] = nil

			-- trigger completed callbacks when all properties finish animating
			local callbackList = completedCallbacks[key]
	...
	for setter, spring in valueSprings do
		if spring:canSleep() then
			setter(spring.rawGoal)
			valueSprings[setter] = nil

			-- trigger completed callbacks
			local callbackList = completedCallbacks[setter]
			if callbackList then
				completedCallbacks[setter] = nil
				for _, callback in callbackList do
					task.spawn(callback)
				end
			end
		else
			setter(spring:step(dt))
		end
	end
end
...
function spr.target(key: any, dampingRatio: number, frequency: number, properties: {[string]: any})
	...
	local isCamera = isInstance and key:IsA("Camera")
	local targetRecord = (isCamera and springStates_render or springStates_other) :: {[any]: {[string]: any}}

	local state = targetRecord[key]
	if not state then
		state = {}
		targetRecord[key] = state
	end

	for propName, propTarget in properties do
		local propValue
		if isInstance then
			propValue = getProperty(key, propName)
		else
			-- For non-instances, we need a way to get the current value.
			-- This example assumes a 'get' function is provided in a table.
			if typeof(key) == "table" and typeof(key.get) == "function" then
				propValue = key.get(propName)
			else
				error("For non-instance targets, a 'get' function must be provided.", 2)
			end
		end
		...
		if frequency == math.huge then
			if isInstance then
				setProperty(key, propName, propTarget)
			elseif typeof(key) == "table" and typeof(key.set) == "function" then
				key.set(propName, propTarget)
			end
			state[propName] = nil
			continue
		end
	...
	if not next(state) then
		targetRecord[key] = nil
	end
end

function spr.value(initialValue: any, setter: (any) -> ())
	if STRICT_RUNTIME_TYPES then
		assertType(2, "spr.value", "function", setter)
	end
	...
	local internalSetter = function(newValue: any)
		currentValue = newValue
		setter(newValue)
	end

	function valueApi:target(dampingRatio: number, frequency: number, targetValue: any)
		if STRICT_RUNTIME_TYPES then
			assertType(1, "target", "number", dampingRatio)
			assertType(2, "target", "number", frequency)
		end

		if not spring then
			local md = typeMetadata[typeof(targetValue)]
			if not md then
				error("unsupported type: " .. typeof(targetValue), 2)
			end

			spring = md.springType(dampingRatio, frequency, currentValue, targetValue, md)
			valueSprings[internalSetter] = spring
		else
			if not valueSprings[internalSetter] then
				valueSprings[internalSetter] = spring
			end
		end
	...
	function valueApi:stop()
		if valueSprings[internalSetter] then
			valueSprings[internalSetter] = nil
		end
	end

	function valueApi:completed(callback: () -> ())
		if STRICT_RUNTIME_TYPES then
			assertType(1, "completed", "function", callback)
		end

		local callbackList = completedCallbacks[internalSetter]
		if not callbackList then
			callbackList = {}
			completedCallbacks[internalSetter] = callbackList
		end
...
function spr.stop(key: any, property: string?)
	if STRICT_RUNTIME_TYPES then
		assertType(2, "spr.stop", "string|nil", property)
	end

	local state = springStates_other[key] or springStates_render[key]
	if property then
		if state then
			state[property] = nil
		end
		...
		if state then
			springStates_other[key] = nil
			springStates_render[key] = nil
		end

		if valueSprings[key] then
			valueSprings[key] = nil
		end
	end
end

function spr.completed(key: any, callback: ()->())
	if STRICT_RUNTIME_TYPES then
	...
	if callbackList then
		...
		completedCallbacks[key] = {callback}
	end
end
